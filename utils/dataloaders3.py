...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self,
        ...
        if cache_images:
            ...
            fcn = self.cache_images_to_disk if cache_images == 'disk' else self.load_image0
        ...
        if os.path.exists(self.path_image_foreign):
        ...
        if self.cache_foreign:
            ...
            self.num_foreign = len(self.list_npy_foreign)
            ...
            # if np.random.randint(2):
                ...
                if 'point' in name_foreign or max(image_foreign.shape) < 10:
                    ...
                    if t <= 256:
                        ...
                        low_random = 2 # 4
                        high_random = 8+1 # 16+1
                    elif t <= 256+64+16:
                        low_random = 4 # 16
                        high_random = 16+1 # 64+1
                    else:
                        low_random = 8 # 64
                        high_random = 32+1 # 256+1
                    # low_random = 4
    ...
    def load_image(self, i):
        # Loads 1 image from dataset index 'i', returns (im, original hw, resized hw)
        im, f, fn, label = self.ims[i], self.im_files[i], self.npy_files[i], self.labels[i].copy()
        #
        if im is None:  # not cached in RAM
            if fn.exists():  # load npy
                im = np.load(fn)
        ...
        # if np.random.randint(2):
        if self.augment:
            im, label_generated = self.paste_foreign_to_background(im)
            label = np.concatenate([label, label_generated], 0)
        return im, (h0, w0), (h, w), label  # im, hw_original, hw_resized
